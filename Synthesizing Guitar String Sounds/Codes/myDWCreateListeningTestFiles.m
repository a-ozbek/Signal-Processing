...
for i=1:size(noteFreqs,2)
    for j=i+1:size(noteFreqs,2)
        wavwrite([myDWSoundGenerator(noteFreqs{i},nD,0.5,0.5,Fs); pauseDuration; myDWSoundGenerator(noteFreqs{j},nD,0.5,0.5,Fs)], Fs, strcat('DW',noteNames{i},'v',noteNames{j}));         
    end
end
